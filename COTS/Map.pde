...
class COTSMap_GPU {
  ...
  PShader myShader;
  PShape[] tris = new PShape[nSegs];
  PApplet parent;
  
  COTSMap_GPU() {}
  
  COTSMap_GPU(PApplet p, pt A1, pt B1, pt C1, pt D1)      // creates map and sets its parameters
    {
      parent = p;
    ...
    COTSMap_GPU(PApplet p, pt A1, pt B1, pt C1, pt D1, int cells) // creates map and sets its parameters and cell-count
    {
      parent = p;
      nCells=cells;
      A=P(A1);  B=P(B1);  C=P(C1);  D=P(D1);
  ...
  COTSMap_GPU(PApplet p, pt A1, pt B1, pt C1, pt D1, int cells, boolean reference) // creates map and sets its parameters and cell-count
    {
      parent = p;
      nCells=cells;
      if (reference) {
  ...
  void SetN(int cells) {nCells=cells;}                  // changes cell-count
  
  void UpdateShader() {
      // set shader uniform variables
      myShader.set("A", A.x, A.y);
      myShader.set("F", F.x, F.y);
      myShader.set("scales", mu, mv);
      myShader.set("angles", au, av);
  }
    ...
    void UpdateMap(pt A1, pt B1, pt C1, pt D1)
    {
      A=P(A1);  B=P(B1);  C=P(C1);  D=P(D1);  
      UpdateSpiral();
    }
    
    void UpdateMap(pt A1, pt B1, pt C1, pt D1, int cells)     // recomputes COTS parameter, selecting angle branchings that minimize pops
    {
      nCells=cells;
      A=P(A1);  B=P(B1);  C=P(C1);  D=P(D1);  
      UpdateSpiral();
    ...
    void UpdateMapReference(int np)
    {
      nCells=np;
      UpdateSpiral();
    }
    ...
    for (int n = 0; n < nSegs; n++) {
      tris[n] = createShape();
      tris[n].beginShape(TRIANGLE_STRIP);
      tris[n].noStroke();
      for (int i = 0; i < nSegs; i++)
      {
       tris[n].vertex(i*delta, n*delta, 0);
       tris[n].vertex(i*delta, (n+1)*delta, 0);
      }
      tris[n].endShape();
      //tris.setStroke(color(255));
      //tris.setStrokeWeight(5);
    }
    }
...
class COTSMap 
  ...
  COTSMap(pt A1, pt B1, pt C1, pt D1, int np, boolean reference) // creates map and sets its parameters and cell-count
    {
    if (reference) {
     A=A1;  B=B1;  C=C1;  D=D1; 
    }
  ...
  void COTSupdateMap(pt A1, pt B1, pt C1, pt D1, int np)     // recomputes COTS parameter, selecting angle branchings that minimize pops
    ...
    void COTSupdateMapReference(int np)     // recomputes COTS parameter, selecting angle branchings that minimize pops
    {
    n=np;
    mu = spiralScale(A,B,D,C);
    mv = spiralScale(A,D,B,C);
  ...
  void showLabels() {pen(black,1); showId(CM.A,"a"); showId(CM.B,"b"); showId(CM.C,"c"); showId(CM.D,"d");  showId(CM.F,"f"); }
 ...
 pt Inverse(pt M) 
    ...
    void showLine(pt p1, pt p2) {
      int samples = 30;
      noFill();
      beginShape();
      for (int i = 0; i < samples+1; i++) {
        float t = (float)i / samples;
        pt P = P(t, p1, 1.0-t, p2);
        v(ImageOf(P));
      }
      endShape();
    }
    
    void showTriangle(pt p1, pt p2, pt p3) {
      pen(red, 3);
      showLine(p1, p2);
      showLine(p2, p3);
      showLine(p3, p1);
    ...
    void showTriangleTest() {
      //randomSeed(0);
     for (int i = 0; i < 100; i++) {
      showTriangle(P(random(1.0), random(1.0)), P(random(1.0), random(1.0)), P(random(1.0), random(1.0)));
     }
    }
